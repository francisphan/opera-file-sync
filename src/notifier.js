...
class Notifier {
   ...
   * previewed (scripts/preview-front-desk-report.js). Posting Masters are not
   * part of this report — they go out separately via sendPostingMastersReport.
  ...
  buildDailyFrontDeskReport(reportData) {
    const { date, inHouse, departures, arrivalsToday, arrivalsTomorrow, arrivalsDayAfter = [] } = reportData;
  ...
  /**
   * Send the posting-masters report: the 9000-series charge-tracking accounts
   * (not real guest stays). Split out of the daily front desk report onto its
   * own reduced cadence (twice a week) since this list rarely changes.
   * @param {Object} reportData - From queryFrontDeskReport() (uses date + postingMasters)
   * @param {string} [toOverride] - Recipient override (defaults to POSTING_MASTERS_EMAIL_TO / FRONT_DESK_EMAIL_TO)
   * @param {string} [ccOverride] - CC override (defaults to POSTING_MASTERS_EMAIL_CC)
   */
  async sendPostingMastersReport(reportData, toOverride, ccOverride) {
    const to = toOverride || process.env.POSTING_MASTERS_EMAIL_TO || this.frontDeskEmailTo || this.emailTo;
    const cc = ccOverride || process.env.POSTING_MASTERS_EMAIL_CC || null;
    if (!this.emailEnabled || !to) {
      logger.info('Posting masters report: email disabled or no recipient configured — skipping');
      return false;
    }

    const { date, postingMasters = [] } = reportData;

    if (postingMasters.length === 0) {
      logger.info('Posting masters report: none on property — skipping');
      return false;
    }

    const subject = `Posting Masters Report — ${date}`;
    ...
    // ── Plain-text fallback ──
    const textLines = [
      `Posting Masters Report — ${date}`,
      `${postingMasters.length} charge account(s) (9000-series, not real stays)`,
      ``
    ];
    postingMasters.forEach(g => {
      textLines.push(`  - ${g.firstName} ${g.lastName} | Villa: ${formatVilla(g.villa) || '—'} | ${g.checkIn}→${g.checkOut}`);
    });
    const textBody = textLines.join('\n');
    ...
    let htmlBody = `
      <h2 style="margin-bottom:4px">Posting Masters Report</h2>
      <p style="color:#666;margin-top:0"><strong>Date:</strong> ${date}</p>
    ...
    const csvRows = ['Name,Email,Villa,Check-in,Check-out,Notes'];
    for (const g of postingMasters) {
      csvRows.push([
        `${g.firstName} ${g.lastName}`,
        g.email || '',
        formatVilla(g.villa) || '',
    ...
    const attachments = [{
      filename: `posting-masters-${date}.csv`,
      content: csvRows.join('\n'),
      contentType: 'text/csv'
    }];

    const sent = await this._sendEmailToRecipients(to, subject, textBody, htmlBody, attachments, cc);
    logger.info(`Posting masters report sent to ${to}${cc ? ` (cc ${cc})` : ''}: ${postingMasters.length} account(s)`);
    return sent;
  }
